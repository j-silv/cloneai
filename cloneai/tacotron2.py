...
def run(in_dir, out_dir, train_split, val_split, seed):
    print(in_dir, out_dir, train_split, val_split, seed)
    
        
    # We can also print some aspects of the data just to remind us of
    # the shapes and the input to the tacotron2 model.
    dataset = AudioDataset("/content/cloneai/data/processed/1-scott")

    print(f"{dataset.audio_files[0]=}",
        f"{dataset.transcriptions[0]=}",
        f"{dataset.token_lengths=}",
        f"{dataset.idx_sorted_samples=}",
    ...
    print(dataset.token_lengths[0])
    plot_mel_spec(dataset.mels[0], dataset.sr)

    print(dataset.token_lengths[-1])
    plot_mel_spec(dataset.mels[-1], dataset.sr)
