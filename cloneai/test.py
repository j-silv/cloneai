...
def test_split():
    ...
    #     split.write(infile, outfile, ss=start, to=end, verbose=True, sample_rate_hz=44100, accurate=False)

    # ----------------------------------------------------------------------------------------

    infile = "./data/test/scott.aac"
    silences = split.get_silences(infile, silence_db=-30, min_silence_s=1.0, verbose=False)
    # print(f"{silences=}")
